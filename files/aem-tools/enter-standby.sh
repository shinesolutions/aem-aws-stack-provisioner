...
then
    ...
    aws autoscaling enter-standby --instance-ids "${instance_id}" --auto-scaling-group-name "${group_name}" --should-decrement-desired-capacity

else

    echo "Instance is in ${lifecycle_state} State. Can not Enter Standby."
